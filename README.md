...
Then: reload / wait for Sync → enable **Obsidinect** → paste an API key.
...
1. Open **Obsidinect chat** from the ribbon or the command palette.
...
iOS Obsidian **cannot spawn processes**. Obsidinect is a smaller, mobile-first plugin inspired by that UX only. It does not clone Claudian and does not require those CLIs.
